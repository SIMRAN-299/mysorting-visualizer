...
export default class SortingVisualizer extends React.Component {
  ...
  resetArray() {
    const array = [];
    array.push(530);
    for (let i = 0; i < this.state.bar-1; i++) {
  ...
  mergeSort() {
    if (sorting_already_running === 0) {
      //condition for checking if algo is running or not
      ...
      for (let i = 0; i < animations.length; i++) {
        ...
        if (isColorChange) {
          ...
          setTimeout(() => {
            ...
            const barOneStyle = arrayBars[barOneIdx].style;
            barOneStyle.height = `${newHeight/heightDivisor}%`;
          }, i * this.state.speed);
        }
      }
      setTimeout(() => {
      ...
      alert(
        "Sorting already running.You can't run another algorithm when one algorithm is running"
      );
    }
  }
      ...
      for (let i = 0; i < animations.length; i++) {
        ...
        if (animations[i][0] == "swap") {
          ...
          if (barone == bartwo) {
            ...
            setTimeout(() => {
              barOneStyle.height = `${height1/heightDivisor}%`;
              barTwoStyle.height = `${height2/heightDivisor}%`;
            }, timer * this.state.speed);
            timer++;
          }
        }
        ...
        <div className="buttons">
          ...
          <div className="margin-left">
            ...
            <input
              ...
              min="10"
              max="45"
              onChange={() => this.changeNumberofarray()}
            />
          </div>
        </div>
            ...
            <div className="array-container">
                <div className="array-box">
                    {array.map((value, idx) => (//this contains different divs
                      <div
                        className="array-bar"
                        key={idx}
                        style={{
                          ...
                          height: `${value/heightDivisor}%`,
                        
                          width: `${this.state.mywidth/25}%`,
         ...
         <div className="buttons below" >
          <button className="reload" onClick={() => this.res()}>
            {" "}
            RELOAD{" "}
          </button>
